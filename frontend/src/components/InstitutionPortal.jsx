...
const AddCertificateForm = ({ onLogout }) => {
   ...
   return (
      <div className="bg-[#FDFEFF] min-h-[80vh] flex flex-col items-center justify-center font-inter">
        <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-2xl mx-auto relative">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              {/* Logo can be imported if available */}
              <span className="font-bold text-[#0A2342] text-xl">CertiSure</span>
            </div>
            <button onClick={onLogout} className="text-[#F9A826] font-bold px-4 py-2 rounded-lg border border-[#F9A826] hover:bg-[#F9A826] hover:text-white transition-colors">Logout</button>
          </div>
          <h2 className="text-2xl font-bold text-[#0A2342] mb-4">Add New Certificate Record</h2>
          <div className="bg-[#F9A826]/10 border-l-4 border-[#F9A826] p-4 rounded-lg mb-6 text-[#0A2342]">
            Note: This will add the certificate record to both the database and the blockchain. The transaction is permanent.
          </div>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-[#0A2342]">Student Full Name</label>
              <input type="text" name="student_name" value={formData.student_name || ''} onChange={handleInputChange} className={inputStyles} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#0A2342]">Roll Number / ID</label>
              <input type="text" name="roll_number" value={formData.roll_number || ''} onChange={handleInputChange} className={inputStyles} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#0A2342]">Course / Degree Name</label>
              <input type="text" name="course_name" value={formData.course_name || ''} onChange={handleInputChange} className={inputStyles} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#0A2342]">Grade / CGPA</label>
              <input type="text" name="grade" value={formData.grade || ''} onChange={handleInputChange} className={inputStyles} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#0A2342]">Issue Date</label>
            ...
            <div className="md:col-span-2">
              ...
              {success && <p className="text-[#16A34A] text-sm">{success}</p>}
              <button 
                type="submit" 
                disabled={isLoading}
                className={`w-full font-bold py-3 px-4 rounded-lg transition-colors ${
                  isLoading 
                    ? 'bg-[#0A2342]/60 cursor-not-allowed' 
                    : 'bg-[#0A2342] text-white hover:bg-[#07162a]'
                }`}
              >
